# BigQuery - datasets, EDA, Ml models
...
/* Predict purchase per user using model */
SELECT
  fullVisitorId,
  SUM(predicted_label) as total_predicted_purchases
FROM
...
/* Improve model performance with Feature Engineering - create secondmodel with additional features */
...
WITH params AS (
  ...
  /* Evaulate model performance - perf criteria: RMSE < 10*/
SELECT
...
/* Predict Taxi Fare Fare Amount using ML model */
SELECT
*
FROM
...
### Explore BQ Public data
bigquery-public-data dataset - Explorer panel >> click +Add data > Star a project by name > Enter Project Name, then write the bigquery-public-data name > Click Star >>> The bigquery-public-data project is listed in the Explorer section.

## Explore NCAA Basketball data
```sql
/* determine available seasons and games */
SELECT
  season,
...
/* see which features provided the most value to the model by inspecting the weights */
SELECT
  category,
  weight
FROM
...
/* find biggest upset for  2017 tournament according to the model=> look where model predicts with 80%+ confidence and gets it WRONG */
SELECT
...
/* building a second ML model using newly provided and detailed features */
...
/* get all possible team games in the tournament */
SELECT
  NULL AS label,
...
## Challenge Lab Engineer Data for Predictive Modeling with BigQuery ML

```sql
/* clean the data and populate training data table */
CREATE OR REPLACE TABLE `taxirides.taxi_training_data_286` AS
SELECT
  pickup_datetime,
  pickup_longitude AS pickuplon,
  pickup_latitude AS pickuplat,
  ...
  dropoff_latitude AS dropofflat,
  passenger_count AS passengers,
  (tolls_amount + fare_amount) AS fare_amount_286
FROM
...
/* create new model - see use of transform */
CREATE OR REPLACE MODEL `taxirides.fare_model_427`
TRANSFORM(
  fare_amount_286,
...
OPTIONS(
  ...
  input_label_cols=['fare_amount_286']
) AS
SELECT * FROM `taxirides.taxi_training_data_286`;

/* evaluate model - check if RMSE < 10*/
SELECT
  SQRT(mean_squared_error) AS rmse
FROM
  ML.EVALUATE(MODEL `taxirides.fare_model_427`,
    (SELECT * FROM `taxirides.taxi_training_data_286`)
  );

/* Batch predictions */
CREATE OR REPLACE TABLE `taxirides.2015_fare_amount_predictions` AS
SELECT * FROM ML.PREDICT(MODEL `taxirides.fare_model_427`, 
